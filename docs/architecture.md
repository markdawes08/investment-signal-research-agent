...
`llm_design.py` defines prompt version `research-design-v1` and schema version `research-proposal-v1`. Its strict object schema requires candidate and parent IDs, research claim, signal, lookback, selection count, evidence claims and excerpts, assumptions, concise decision rationale, limitations, parameter basis, and any adaptation rationale. The proposal envelope records `propose`, `revise`, or `defer`, its selected candidate ID, and a concise decision or deferral reason. It requests structured decisions, not private chain-of-thought transcripts. Extra fields, wrong types, duplicate identifiers, and values outside declared bounds are rejected deterministically.
...
The audit records application-observed requested and actual model identifiers, response IDs where available, call attempts and actual-call flags, prompt/schema versions, reproducible context hashes and sanitized contexts, structured candidate output, observations, revisions, and selected specification. A model-provided JSON field cannot certify that a real request happened. Mock-adapter evidence and real-provider evidence are reported separately.
